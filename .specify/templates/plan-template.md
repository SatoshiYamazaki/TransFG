...
- Plan smoke test on a tiny subset (2 batches/split) including FP16/distributed params if used.
- Define evaluation split, resolution, and metrics (top-1 accuracy + loss) tied to the planned checkpoint.
- Declare GPU count, torch.distributed launch shape, and ensure data/output paths are parameterized (not hard-coded).
- Enumerate variant axes (ViT size, backbone: CLIP/SigLIP2/DINOv2, part attention on/off, patch/resolution) and how comparability will be enforced (shared preprocessing, normalization source, eval metric).
...
- Provide miniconda environment details (environment.yml or export) and how it is applied in runs.
